...
char *CSdpHandler::make_relay_session_id(const char *username, const char *relay){
	...
	void *src=&res;
	//b64_encode2(src, sizeof(long long int), tmp, sizeof(tmp));
	return osip_strdup(tmp);
}

char * CSdpHandler::int_2char(int a){
	char *p=(char*)osip_malloc(16);
	snprintf(p,16,"%i",a);
	return p;
}

/* return the value of attr "field" for payload pt at line pos (field=rtpmap,fmtp...)*/
char *CSdpHandler::sdp_message_a_attr_value_get_with_pt(sdp_message_t *sdp,int pos,int pt,const char *field)
{
...
char *CSdpHandler::sdp_message_a_attr_value_get(sdp_message_t *sdp,int pos,const char *field)
{
	int i;
	sdp_attribute_t *attr;
	for (i=0;(attr=sdp_message_attribute_get(sdp,pos,i))!=NULL;i++){
 ...
 int CSdpHandler::_sdp_message_get_a_ptime(sdp_message_t *sdp, int mline){
	int i,ret;
	sdp_attribute_t *attr;
...
int
CSdpHandler::sdp_payload_init (sdp_payload_t * payload)
{
	memset (payload, 0, sizeof (sdp_payload_t));
	return 0;
}

sdp_context_t *CSdpHandler::sdp_handler_create_context(sdp_handler_t *handler, const char *localip, const char *username, const char *relay){
	sdp_context_t *ctx=(sdp_context_t *)osip_malloc(sizeof(sdp_context_t));
...
void CSdpHandler::sdp_context_set_user_pointer(sdp_context_t * ctx, void* up){
	ctx->reference=up;
}

void *CSdpHandler::sdp_context_get_user_pointer(sdp_context_t * ctx){
	return ctx->reference;
}

int CSdpHandler::sdp_context_get_status(sdp_context_t* ctx){
	return ctx->negoc_status;
}

/* generate a template sdp */
sdp_message_t *CSdpHandler::
sdp_context_generate_template (sdp_context_t * ctx)
{
 ...
 void CSdpHandler::add_relay_info(sdp_message_t *sdp, int mline, const char *relay, const char *relay_session_id){
...
void
CSdpHandler::sdp_context_add_payload (sdp_context_t * ctx, sdp_payload_t * payload, char *media)
{
...
void
CSdpHandler::sdp_context_add_audio_payload (sdp_context_t * ctx, sdp_payload_t * payload)
{
	sdp_context_add_payload (ctx, payload, "audio");
}

void
CSdpHandler::sdp_context_add_video_payload (sdp_context_t * ctx, sdp_payload_t * payload)
{
	sdp_context_add_payload (ctx, payload, "video");
}

char *
CSdpHandler::sdp_context_get_offer ( sdp_context_t * ctx)
{
	sdp_message_t *offer;
...
/* refuse the line */
void CSdpHandler::refuse_mline(sdp_message_t *answer,char *mtype,char *proto, int mline)
{
...
char * CSdpHandler::parse_relay_addr(char *addr, int *port)
{
	char *semicolon=NULL;
	char *p;
...
char *
CSdpHandler::sdp_context_get_answer ( sdp_context_t *ctx,sdp_message_t *remote)
{
	...
	if (ctx->localip==NULL) {
		...
		ctx->localip=(char*)osip_malloc(128);
		eXosip_guess_localip(strchr(tmp,':') ?  AF_INET6 : AF_INET,ctx->localip,128);
	}
	//else 
	//	eXosip_trace(OSIP_INFO1,("Using firewall address in sdp."));
...
void
CSdpHandler::sdp_context_read_answer (sdp_context_t *ctx, sdp_message_t *remote)
{
	char *mtype;
	char *proto, *port, *pt;
	int i, j,err;
...
void CSdpHandler::sdp_context_free(sdp_context_t *ctx){
