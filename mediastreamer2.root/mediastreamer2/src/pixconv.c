...
int ms_pix_fmt_to_ffmpeg(MSPixFmt fmt){
	switch(fmt){
		...
		case MS_YUY2:
			return PIX_FMT_YUYV422; 
		case MS_RGB565:
			return PIX_FMT_RGB565;/* <- same as MS_YUYV */
		default:
...
static void pixconv_preprocess(MSFilter *f){
	//PixConvState *s=(PixConvState*)f->data;
	//s->running=TRUE;
}
		...
		if (s->in_fmt==s->out_fmt){
			...
			if (s->sws_ctx==NULL){
				s->sws_ctx=sws_getContext(s->size.width,s->size.height,
					s->in_fmt,s->outbuf.w,s->outbuf.h,
