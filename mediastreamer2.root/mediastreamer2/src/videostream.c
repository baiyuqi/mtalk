...
int video_stream_start (VideoStream *stream, RtpProfile *profile, const char *remip, int remport,
	int rem_rtcp_port, int payload, int jitt_comp, MSWebCam *cam,HWND hWnd){
	PayloadType *pt;
	...
	ms_filter_call_method(stream->source,MS_FILTER_SET_VIDEO_HWND,&hWnd);
	...
	disp_size.width=MS_VIDEO_SIZE_QCIF_W;
	disp_size.height=MS_VIDEO_SIZE_QCIF_H;
	tmp=1;
	...
	ms_filter_call_method(stream->output,MS_VIDEO_OUT_ENABLE_MIRRORING,&tmp);
	//ms_filter_call_method(stream->output,MS_VIDEO_OUT_SET_CORNER,&stream->corner);
	ms_filter_call_method(stream->output,MS_VIDEO_OUT_SET_NATIVE_WINDOW_ID,&hWnd);
...
VideoStream * video_preview_start(MSWebCam *device, MSVideoSize disp_size,HWND vhwnd){
	VideoStream *stream = (VideoStream *)ms_new0 (VideoStream, 1);
	...
	ms_filter_call_method(stream->source,MS_FILTER_SET_VIDEO_SIZE,&vsize);
	ms_filter_call_method(stream->source,MS_FILTER_SET_FPS,&fps);
	ms_filter_call_method(stream->source,MS_FILTER_SET_VIDEO_HWND,&vhwnd);
	ms_filter_call_method(stream->source,MS_FILTER_SET_VIDEO_PREVIEW,&priview);
	stream->source->desc->preprocess(stream->source);
	return stream;
}
