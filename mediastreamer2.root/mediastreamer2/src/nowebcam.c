...
static mblk_t *jpeg2yuv(uint8_t *jpgbuf, int bufsize, MSVideoSize *reqsize){
	...
	AVCodec *pt = avcodec_find_decoder(CODEC_ID_MJPEG);
	avcodec_get_context_defaults(&av_context);
	//ms_error("test:%d;%d;%d;%d",pt->id,pt->type,av_context.codec_id,av_context.codec_type);
	
	av_context.codec_id = pt->id;
	av_context.codec_type = pt->type;
	av_context.width=352;
	av_context.height=288;

	if(pt==NULL) return NULL;
	if (avcodec_open(&av_context,pt)<0){
...
static const char *def_image_path="images/" NOWEBCAM_JPG ".jpg";


mblk_t *ms_load_nowebcam(MSVideoSize *reqsize, int idx){
	char tmp[256];
	if (idx<0)
		snprintf(tmp, sizeof(tmp), "images/%s.jpg", NOWEBCAM_JPG);
	else
...
void static_image_preprocess(MSFilter *f){
	...
	snprintf(tmp, sizeof(tmp), "images/%s.jpg", NOWEBCAM_JPG);
	if (d->pic==NULL){
		d->pic=ms_load_jpeg_as_yuv(tmp,&d->vsize);
	}
}
